...
from lib.loss.disentangling_loss import DisentanglingLoss
from lib.loss.rpn_3d_multi_view import RPN_3D_multi_view_loss
from lib.loss.rpn_3d_multi_video import RPN_3D_multi_video
from lib.loss.rpn_3d_front import RPN_3D_loss
...
def main(argv):
    ...
    # criterion_det = RPN_3D_multi_view_loss(conf)
    # criterion_det = RPN_3D_multi_video(conf)
    # criterion_det = DisentanglingLoss(conf)
    ...
    for iteration in range(start_iter, conf.max_iter):
        # next iteration
        if conf.flow:
            iterator, images, flows, imobjs = next_iteration(dataset.loader, iterator, conf)
        else:
            iterator, images, imobjs = next_iteration(dataset.loader, iterator, conf)
        ...
        # forward
        if conf.flow:
            cls, prob, bbox_2d, bbox_3d, feat_size = rpn_net([images, flows])
        else:
            cls, prob, bbox_2d, bbox_3d, feat_size = rpn_net(images[:, :3, ::])
        # loss
