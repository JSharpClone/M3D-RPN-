...
class RPN(nn.Module):
    ...
    def __init__(self, phase, base, conf):
        ...
        self.num_rows = int(min(conf.bins, calc_output_size(conf.test_scale[0], conf.feat_stride)))
        ...
        if self.flow:
            self.prop_feats_loc = nn.Sequential(
                LocalConv2d(self.num_rows, self.base[-1].num_features+2, 512, 3, padding=1),
                nn.ReLU(inplace=True)
            )
        elif self.video_det:
            pass
    ...
    def extract_feature(self, x):
        if self.video_det:
            ...
            im = self.base(im)
            prev_im = self.base(prev_im)
            x = torch.cat([im, prev_im], dim=1)
            # y = torch.cat([prev_im, prev_im], dim=1)
        else:
            # resnet
            x = self.base(x)
    ...
    def forward(self, x):
        ...
        if not self.video_det:
            ...
            cls_loc = self.cls_loc(prop_feats)

            # bbox 2d
            bbox_x_loc = self.bbox_x_loc(prop_feats)
            bbox_y_loc = self.bbox_y_loc(prop_feats)
            bbox_w_loc = self.bbox_w_loc(prop_feats)
            bbox_h_loc = self.bbox_h_loc(prop_feats)

            # bbox 3d
            bbox_x3d_loc = self.bbox_x3d_loc(prop_feats)
            bbox_y3d_loc = self.bbox_y3d_loc(prop_feats)
            bbox_z3d_loc = self.bbox_z3d_loc(prop_feats)
            bbox_w3d_loc = self.bbox_w3d_loc(prop_feats)
            bbox_h3d_loc = self.bbox_h3d_loc(prop_feats)
            bbox_l3d_loc = self.bbox_l3d_loc(prop_feats)
            bbox_rY3d_loc = self.bbox_rY3d_loc(prop_feats)
